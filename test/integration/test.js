...
test.beforeEach(async (t) => {
  ...
  await t.context.forwardEmail.listen();
});
...
test('creates 100 simultaneous connections (w/o rate limiting)', async (t) => {
  const forwardEmail = new ForwardEmail({ limiter: false, dkim });
  const port = await getPort();
  forwardEmail.server.listen(port);
  await Promise.all(
...
// TODO: consider if this should be used since Spam Scanner
// already runs these tests
