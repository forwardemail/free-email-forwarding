...
> :warning: If you are using Google Apps, go over your admin panel [Apps > G Suite >Settings for Gmail > Advanced settings](https://admin.google.com//AdminHome#ServiceSettings/service=email&subtab=filters) and make sure "Allow users to send mail through an external SMTP server..." is checked. If not, check it and save the settings. There will be some delay for this change to be active to wait for ~5 minutes.
